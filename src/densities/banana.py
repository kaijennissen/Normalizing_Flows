...
def banana_sample(prng_key, sample_shape):
    """pdf(x1,x2)=N(x1|(1/4)*x2**2,1)N(x2|0,4)"""
    x2 = tfd.Normal(loc=0, scale=2.0).sample(seed=prng_key, sample_shape=sample_shape)
    x1 = tfd.Normal(loc=x2 ** 2 / 4, scale=1.0).sample(seed=prng_key)
...
if __name__ == "__main__":
    ...
    sns.histplot(x=samples[:, 0], y=samples[:, 1], bins=50, pthresh=0.1, cmap="mako")
